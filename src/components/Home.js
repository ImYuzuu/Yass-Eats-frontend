...
import {
  Box, Flex, Heading, Input, IconButton, Button, Text, VStack, HStack, Image, Badge, SimpleGrid, Divider, Avatar
...
const mieuxNotes = [
  ...
  { name: "Hugo's Tacos", img: '/img/hugostacos.jpg', distance: 3.6, note: 4.6 },
...
export default function Home() {
  return (
    <Box bg="gray.50" minH="100vh" pb={10}>
      {/* Header */}
      <Flex align="center" justify="space-between" bg="white" px={6} py={3} boxShadow="sm">
        <HStack spacing={3}>
          <IconButton icon={<FaBars />} variant="ghost" aria-label="Menu" />
          <Image src="/img/logo.png" alt="Yass'Eats" h="40px" />
        </HStack>
        <HStack spacing={8}>
          <Button variant="ghost" colorScheme="green">Accueil</Button>
          <Button variant="ghost">Commandes</Button>
          <Button variant="ghost">Autour de moi</Button>
          <Button variant="ghost">Profil</Button>
        </HStack>
        <Box>
          <Input value="10 rue du moulin, 75000 Paris" size="sm" w="250px" readOnly />
        </Box>
      </Flex>
      ...
      <Flex justify="center" mt={8}>
        <Button leftIcon={<FaShoppingCart />} colorScheme="green" size="lg" borderRadius="lg">
          COMMANDER
        </Button>
      </Flex>
    </Box>
